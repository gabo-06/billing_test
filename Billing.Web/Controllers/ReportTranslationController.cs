...
{
    public class ReportTranslationController : Controller
    {
        ...
        public ReportTranslationController()
        { 
            this._HtmlViewRenderer = new HtmlViewRenderer();
            this.StandardPdfRenderer = new StandardPdfRenderer();
        }

        public PartialViewResult listaTranslation()
        {
            List<SP_REPORT_TRANSLATION_Result> Listado;
            Listado = model.ReporteTranslation();
            return PartialView(Listado);
        }


        public ActionResult ExportToWordTranslation(String invoice = "", String DOI = "", String patient = "")
        {
            ...
            strBody = strBody + "<tr><td colspan=4 style='text-align:center; font-weight:bold;width:100px;font-size: 25px;font-family: sans-serif;'>TRANSLATION REPORTS</td></tr></table>";
            ...
            string nombre_archivo = "Translation_reports-" + fechaActual; 

            //string htmlString = this.RenderRazorViewToString(@"TestPartialViewWithModel", DetalleDeFactura);
            string htmlString = this.RenderRazorViewToString(@"listaTranslation", Listado);
